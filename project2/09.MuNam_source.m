...
IDP = zeros(size(score_page));

for i = 1:length(alpha)
    [~,IDP(:,i)]=sort(score_page(:,i),'descend');
    %disp(webpage(idp(1:5))')
end
...
% in-degree score ranking
[~,ida] = sort(score_in,'descend');
disp(webpage(ida(1:5))')

% out-degree score ranking
[~,idh] = sort(score_out,'descend');
disp(webpage(idh(1:5))')
...
[~,idu]=sort(u1,'descend');
[~,idv]=sort(v1,'descend');
disp(webpage(idu(1:5))')
disp(webpage(idv(1:5))')
